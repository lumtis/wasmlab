...
import { Addr, InstantiateMsg, VaultInfo, ExecuteMsg, LockMsg, TriggerUnlockMsg, CompleteUnlockMsg, QueryMsg, MigrateMsg, LockedResponse, Timestamp, Uint64, UnlockingResponse, UnlockingTokens, VaultInfoResponse } from "./Vault.types";
export interface VaultReadOnlyInterface {
  ...
  locked: ({
    ...
    addr: Addr;
  }) => Promise<LockedResponse>;
  unlocking: ({
    addr
  }: {
    addr: Addr;
  }) => Promise<UnlockingResponse>;
}
...
export interface VaultInterface extends VaultReadOnlyInterface {
  ...
  completeUnlock: (fee?: number | StdFee | "auto", memo?: string, funds?: Coin[]) => Promise<ExecuteResult>;
}
export class VaultClient extends VaultQueryClient implements VaultInterface {
  ...
  completeUnlock = async (fee: number | StdFee | "auto" = "auto", memo?: string, funds?: Coin[]): Promise<ExecuteResult> => {
    return await this.client.execute(this.sender, this.contractAddress, {
      complete_unlock: {}
    }, fee, memo, funds);
  };
